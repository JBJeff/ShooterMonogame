...
namespace Silent_Shadow.Models.AI.Navigation
{
	public class NavMesh
	{
		...
		public void BuildGraph(float gridSize = 1f)
		{
			Nodes.Clear();
			...
			foreach (var triangle in Triangles)
			{
				...
				Vector2 alignedCentroid = AlignToGrid(centroid, gridSize);

				Node node = new Node(alignedCentroid);
				Nodes.Add(node);
			}
		...
		private Vector2 AlignToGrid(Vector2 position, float gridSize)
		{
			float x = (float)Math.Round(position.X / gridSize) * gridSize;
			float y = (float)Math.Round(position.Y / gridSize) * gridSize;
