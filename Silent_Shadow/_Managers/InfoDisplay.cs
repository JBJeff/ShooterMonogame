...
namespace Silent_Shadow.GUI
{
    public class InfoDisplay
    {
        ...
        (SpriteFont font, Texture2D reloadIcon, Vector2 position)
        {
            ...
            spriteKnife = Globals.Content.Load<Texture2D>("Sprites/knife");
            this.font = font;
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            ...
            if (weapon != null && !(weapon is Knife))
            {
                ...
                else if (weapon is Knife)
                {
                    // Position des Waffensymbols festlegen (links von der Munition)
                    Vector2 iconPosition = new Vector2(position.X - 90, position.Y);
                    spriteBatch.Draw(spriteKnife, iconPosition, null, Color.White, 0f, Vector2.Zero, 0.5f, SpriteEffects.None, 0f);
                }
